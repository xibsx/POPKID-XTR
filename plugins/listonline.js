...
async (cmd, mek, m, { from, quoted, isGroup, isAdmins, isCreator, fromMe, reply }) => {
    try {
        ...
        const groupData = await cmd.groupMetadata(from);
        const presencePromises = [];

        // Request presence updates for all participants
        for (const participant of groupData.participants) {
            presencePromises.push(
                cmd.presenceSubscribe(participant.id)
                    .then(() => {
                        // Additional check for better detection
                        return cmd.sendPresenceUpdate('composing', participant.id);
                    })
            );
        }
        ...
        cmd.ev.on('presence.update', presenceHandler);
        ...
        const checkOnline = async () => {
            ...
            if (checksDone >= checks) {
                ...
                cmd.ev.off('presence.update', presenceHandler);
